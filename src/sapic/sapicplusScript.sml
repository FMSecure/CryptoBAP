...
(* Added definition for substitution for ProcessAction *)
val substitution_to_processAction_def =
Define`
      (substitution_to_processAction S a = (case a of
                                       (Rep) => Rep
                                     | (New _) => New _
                                     | (ChIn (SOME t1) t2) => ChIn (SOME (substitution_to_term S t1)) (substitution_to_term S t2)
                                     | (ChOut (SOME t1) t2) => ChOut (SOME (substitution_to_term S t1)) (substitution_to_term S t2)
                                     | (ChIn (NONE) t) => ChIn NONE (substitution_to_term S t)
                                     | (ChOut (NONE) t) => ChOut NONE (substitution_to_term S t)
                                     | (Event (Fact s ts)) => Event (Fact s (MAP (substitution_to_term S) ts))
                                     | (Insert t1 t2) => Insert (substitution_to_term S t1) (substitution_to_term S t2)
                                     | (Delete t) => Delete (substitution_to_term S t)
                                     | (Lock t) => Lock (substitution_to_term S t)
                                     | (Unlock t) => Unlock (substitution_to_term S t)
                                           ))`;
                                           
(* Added definition for substitution for ProcessComb *)                                           
val substitution_to_processComb_def =
Define`
      (substitution_to_processComb S c = (case c of
                                             Parallel           => Parallel
                                           | NDC                => NDC
                                           | (CondEq t1 t2)     => CondEq (substitution_to_term S t1) (substitution_to_term S t2)
                                           | (Cond t)           => Cond (substitution_to_term S t)
                                           | (Lookup t v)       => Lookup (substitution_to_term S t) v
                                           | (Let t1 t2)        => Let (substitution_to_term S t1) (substitution_to_term S t2)
                                           | (ProcessCall s ts) => ProcessCall s (MAP (substitution_to_term S) ts)
                                          ))`;

(* Added definition for substitution for Process *)                                                           
val substitution_to_process_def = Define`
                                     (substitution_to_process S p = (case p of
                                                                                   ProcessNull => ProcessNull
                                                                     | (ProcessComb c p1 p2)  =>  (ProcessComb (substitution_to_processComb S c) (substitution_to_process S p1) (substitution_to_process S p2))
                                                                     |   (ProcessAction a p')  => ProcessAction (substitution_to_processAction S a) (substitution_to_process S p')
                                                                                ))
                                     `;
...
val sapic_conditional_eq_true_transition_def = Define `
                                                      sapic_conditional_eq_true_transition eq (Config (Ns,St,Pold,Sb,Al)) Ev (Config (Ns',St',Pnew,Sb',Al')) =
(∃Ps P Q t1 t2.
...
val sapic_conditional_eq_false_transition_def = Define `
                                                       sapic_conditional_eq_false_transition eq (Config (Ns,St,Pold,Sb,Al)) Ev (Config (Ns',St',Pnew,Sb',Al')) =
(∃Ps P Q t1 t2.
   (Pold = (BAG_UNION Ps {|ProcessComb (CondEq t1 t2) P Q|})) /\
   (¬(eq t1 t2)) /\
   (Pnew = (BAG_UNION Ps {|Q|})) /\
...
val sapic_delete_transition_def = Define `
                                         sapic_delete_transition eq (Config (Ns,St,Pold,Sb,Al)) Ev (Config (Ns',St',Pnew,Sb',Al')) =
(∃Ps P S t.
   ...
   (Ns = Ns') /\
   (St = State S) /\
   (St' = State (λt'. if eq t' t then NONE else S t')) /\
   (Sb = Sb') /\
   (Al = Al'))`;
...
val sapic_insert_transition_def = Define `
                                         sapic_insert_transition eq (Config (Ns,St,Pold,Sb,Al)) Ev (Config (Ns',St',Pnew,Sb',Al')) =
(∃Ps P S t1 t2.
   ...
   (Ns = Ns') /\
   (St = State S) /\
   (St' = State (λt'. if eq t' t1 then SOME t2 else S t')) /\
   (Sb = Sb') /\
   (Al = Al'))`;
...
val sapic_lock_transition_def = Define `
                                       sapic_lock_transition eq (Config (Ns,St,Pold,Sb,Al)) Ev (Config (Ns',St',Pnew,Sb',Al')) =
(∃Ps P t.
...
val sapic_unlock_transition_def = Define `
                                         sapic_unlock_transition eq (Config (Ns,St,Pold,Sb,Al)) Ev (Config (Ns',St',Pnew,Sb',Al')) =
(?Ps P t.
...
val sapic_lookup_false_transition_def = Define `
                                               sapic_lookup_false_transition eq (Config (Ns,St,Pold,Sb,Al)) Ev (Config (Ns',St',Pnew,Sb',Al')) =
(?Ps P Q S t x.
...
val sapic_lookup_true_transition_def = Define `
                                              sapic_lookup_true_transition eq (Config (Ns,St,Pold,Sb,Al)) Ev (Config (Ns',St',Pnew,Sb',Al')) =
(∃Ps P Q S t t' u x.
   (Pold = (BAG_UNION Ps {|ProcessComb (Lookup t x) P Q|})) /\
...
(* Added definition for T(F, Npub, AX) *)
val is_attacker_term_def = Define 
             ` (is_attacker_term (Con (Name p _)) = (p = PubName)) /\
(is_attacker_term (TVar (Var l _)) = (l = "att")) /\
(is_attacker_term (FAPP (n,(l,p,c)) (t::ts)) <=> (p = Private ∧ (is_attacker_term t) ∧ (is_attacker_term (FAPP (n,((l-1),p,c)) ts))))`  ;

(* K rule *)
(* Adjusted for eq ; Fixed R's domain *)
val sapic_K_transition_def = Define `
                                    sapic_K_transition eq (Config (Ns,St,Pold,Sb,Al)) Ev (Config (Ns',St',Pnew,Sb',Al')) =
(∃R t.
   (Pold = Pnew) /\
   (is_attacker_term R) ∧
   (eq t (substitution_to_term Sb R)) /\
   (Ev = [(Fact KUFact [t])]) /\
   (Ns = Ns') /\
   (St = St') /\
   (Sb = Sb') /\
...
(* In rule *)
(* Adjusted for eq ; Fixed R's and \R'\'s domains *)
val sapic_in_transition_def = Define `
                                     sapic_in_transition eq (Config (Ns,St,Pold,Sb,Al)) Ev (Config (Ns',St',Pnew,Sb',Al')) =
(∃Ps P t x R R'.
...
val sapic_out_transition_def = Define `
                                      sapic_out_transition eq (Config (Ns,St,Pold,Sb,Al)) Ev (Config (Ns',St',Pnew,Sb',Al')) =
(∃Ps P t1 t2 n R S.
   ...
   (is_ground_term t2) /\
   (eq t1 (substitution_to_term Sb R)) /\
...
val sapic_out_in_transition_def = Define `
                                         sapic_out_in_transition eq (Config (Ns,St,Pold,Sb,Al)) Ev (Config (Ns',St',Pnew,Sb',Al')) =
(∃Ps P t t1 t2 x.
...
val sapic_let_true_transition_def = Define `
                                           sapic_let_true_transition eq (Config (Ns,St,Pold,Sb,Al)) Ev (Config (Ns',St',Pnew,Sb',Al')) =
(∃Ps P Q p t h.
   (Pold = (BAG_UNION Ps {|ProcessComb (Let p t) P Q|})) /\
   (* The "grounding" condition is not really needed. *)
   (eq t (substitution_to_term h p)) /\
   (Pnew = (BAG_UNION Ps {|substitution_to_process h P|})) /\
   (Ev = []) /\
   (Ns = Ns') /\
   (St = St') /\
   (Sb = Sb') /\
...
val sapic_let_false_transition_def = Define `
                                            sapic_let_false_transition eq (Config (Ns,St,Pold,Sb,Al)) Ev (Config (Ns',St',Pnew,Sb',Al')) =
(∃Ps P Q p t.
   (Pold = (BAG_UNION Ps {|ProcessComb (Let p t) P Q|})) /\
   (∀h. ~(eq t (substitution_to_term h p))) /\
   (Pnew = (BAG_UNION Ps {|Q|})) /\
   (Ev = []) /\
   (Ns = Ns') /\
   (St = St') /\
...
val sapic_transition_def = Define `
                                  (sapic_transition eq (Config (Ns,St,Pold,Sb,Al)) Ev (Config (Ns',St',Pold',Sb',Al')) =
                                   (∃Ps Pn.
                                      ...
                                      then (T)
                                        ...
                                        (Pold = (BAG_UNION Ps {|Pn|})) ∧ (case Pn of
                                                                          ...
                                                                          | (ProcessComb (CondEq t1 t2) P Q) => ((sapic_conditional_eq_true_transition eq (Config (Ns,St,Pold,Sb,Al)) Ev (Config (Ns',St',Pold',Sb',Al'))) ∨ (sapic_conditional_eq_false_transition eq (Config (Ns,St,Pold,Sb,Al)) Ev (Config (Ns',St',Pold',Sb',Al'))))
                                                                          | (ProcessAction (Delete t) P) => (sapic_delete_transition eq (Config (Ns,St,Pold,Sb,Al)) Ev (Config (Ns',St',Pold',Sb',Al')))
                                                                          | (ProcessAction (Insert t1 t2) P) => (sapic_insert_transition eq (Config (Ns,St,Pold,Sb,Al)) Ev (Config (Ns',St',Pold',Sb',Al')))
                                                                          | (ProcessAction (Lock t) P) => (sapic_lock_transition eq (Config (Ns,St,Pold,Sb,Al)) Ev (Config (Ns',St',Pold',Sb',Al')))
                                                                          | (ProcessAction (Unlock t) P) => (sapic_unlock_transition eq (Config (Ns,St,Pold,Sb,Al)) Ev (Config (Ns',St',Pold',Sb',Al')))
                                                                          | (ProcessComb (Lookup t x) P Q) => ((sapic_lookup_false_transition eq (Config (Ns,St,Pold,Sb,Al)) Ev (Config (Ns',St',Pold',Sb',Al'))) ∨ (sapic_lookup_true_transition eq (Config (Ns,St,Pold,Sb,Al)) Ev (Config (Ns',St',Pold',Sb',Al'))))
                                                                          | (ProcessAction (ChIn (SOME t) (TVar x)) P) => (sapic_in_transition eq (Config (Ns,St,Pold,Sb,Al)) Ev (Config (Ns',St',Pold',Sb',Al')))
                                                                          | (ProcessAction (ChOut (SOME t1) t2) P) => (sapic_out_transition eq (Config (Ns,St,Pold,Sb,Al)) Ev (Config (Ns',St',Pold',Sb',Al')))
                                                                          | (ProcessAction (ChOut (SOME t1) t2) (ProcessAction (ChIn (SOME t) (TVar x)) P)) => (sapic_out_in_transition eq (Config (Ns,St,Pold,Sb,Al)) Ev (Config (Ns',St',Pold',Sb',Al')))
                                                                          | (ProcessComb (Let t1 t2) P Q) => ((sapic_let_true_transition eq (Config (Ns,St,Pold,Sb,Al)) Ev (Config (Ns',St',Pold',Sb',Al'))) ∨ (sapic_let_false_transition eq (Config (Ns,St,Pold,Sb,Al)) Ev (Config (Ns',St',Pold',Sb',Al'))))
...
val _ = Theory.new_constant("position", ``:Process_t -> int -> Process_t``);
...
Pconfig (Process_t # int # sapic_renaming_t # sapic_name_renaming_t)
        `;
...
Pconfig_plus (Process_t # int # sapic_renaming_t # sapic_name_renaming_t)
             `;
