...
val pro = “ProcessAction (New (Name FreshName "49_otp"))
	...
	fun process_to_string pro =
	    let
		fun bracket text = "(" ^ text ^ ")"

		fun process_body_to_string pro =
			...
			in
			    ...
			    then (process_to_string pl)^"\n"^(combinator_to_string c)^"\n"^(process_to_string pr)
	    ...
	    in
		if (is_ProcessNull pro) then "0"
		else bracket (process_body_to_string pro)
	    end
...
(* write Sapic into an explicit file, replacing any previous content *)
fun write_sapic_to_file_path path str =
    let
	val SFile = TextIO.openOut path;
	    
    in
	(TextIO.output (SFile, str); TextIO.flushOut SFile; TextIO.closeOut SFile)
    end;	       

(* Backwards-compatible default used by existing example scripts. *)
fun write_sapic_to_file str =
    write_sapic_to_file_path "Sapic_Translation.txt" str;
