...
fun bir_exp_symbvar_to_symbval vals_lis pred_be =
	...
	let
	 ...
	 in
	     if identical be pred_be then be
	     else ((bir_exp_symbvar_to_symbval vals_lis be) handle e => be)
	 end
...
fun sbir_tree_sapic_process sort_vals tree =
	...
	let
	    ...
	    val true_branch = sbir_tree_sapic_process sort_vals lstr;
	    val false_branch = sbir_tree_sapic_process sort_vals rstr;
	in
	    if (is_leaf lstr) then false_branch
	    else if (is_leaf rstr) then true_branch
	    else if (is_BExp_BinPred be) then
		let
		    val (bop, subexp1, subexp2) = dest_BExp_BinPred be;
		in
		    if (is_BIExp_Equal bop)
		    then mk_ProcessComb (mk_CondEq ((fst (bir_exp_to_sapic_term subexp1)), (fst (bir_exp_to_sapic_term subexp2))), true_branch, false_branch)
		    else mk_ProcessComb (mk_CondEq ((fst (bir_exp_to_sapic_term be)), (fst (bir_exp_to_sapic_term ``BExp_Const (Imm1 1w)``))), true_branch, false_branch)
		end
	    else mk_ProcessComb (mk_CondEq ((fst (bir_exp_to_sapic_term be)), (fst (bir_exp_to_sapic_term ``BExp_Const (Imm1 1w)``))), true_branch, false_branch)
	end
      | VNode ((a,b),str)  =>  (
