...
fun symb_exec_library_block abpfun n_dict bl_dict adr_dict syst =
    let val lbl_tm = SYST_get_pc syst; in
		(case lookup_block_dict bl_dict lbl_tm of
		    NONE =>
			let
			    val syst = bir_symbexec_funcLib.C_Lib syst;
			    val syst = bir_symbexec_funcLib.update_pc syst;
			in
			    abpfun [syst]
			end
		  | SOME bl =>
		...
		fun safe_library_effect effect syst =
		    effect syst handle _ => bir_symbexec_funcLib.C_Lib syst;
...
(* For WireGuard case-study *)
		val systs = if (lib_type = "HMAC_send") then [bir_symbexec_funcLib.HMAC_Send syst]
			    ...
			    else if ((lib_type = "Encryption") orelse (lib_type = "AEAD_ENC")) then [safe_library_effect bir_symbexec_funcLib.Encryption syst]
			    else if ((lib_type = "Decryption") orelse (lib_type = "AEAD_DEC")) then [safe_library_effect bir_symbexec_funcLib.Decryption syst]
			    else if ((lib_type = "DH") orelse (lib_type = "Hash") orelse (lib_type = "KDF")) then [bir_symbexec_funcLib.C_Lib syst]
			    ...
			    else if (lib_type = "C_Lib") then [bir_symbexec_funcLib.C_Lib syst]
			    else [bir_symbexec_funcLib.C_Lib syst];
(*
 ...
 *)
		...
		handle e => raise wrap_exn ("symb_exec_library_block::" ^ term_to_string lbl_tm) e
	    end;
...
fun loop_exit_label lbl_tm =
    raise ERR "loop_exit_label"
      ("loop handling needs an explicit exit label for " ^ term_to_string lbl_tm);

fun state_exec_loop_true bl_dict syst =
    let
	val lbl_tm = SYST_get_pc syst;

	val exit_adr = loop_exit_label lbl_tm;
	    ...
	    val systs_processed = if (is_state_inloop syst) then
			   let
			       ...
			       val systs_processed = abpfun ([syst]);
				       ...
				       val _ = if true then () else print("exit loop "^(term_to_string exit_adr)^"\n");
			   in
			       systs_processed
			   end
		       else if (state_is_running syst) then
			   let
				       val _ = if true then () else print("enter loop "^(term_to_string lbl_tm)^"\n");
...
fun stmt_assigns_link_register stmt =
    if is_BStmt_Assign stmt then
        let
            val (bv, _) = dest_BStmt_Assign stmt;
            val (name, _) = bir_envSyntax.dest_BVar_string bv;
        in
            name = "R30"
        end
...
fun is_self_loop_call_site bl lbl_tm =
    let
        val (_, stmts, est) = dest_bir_block bl;
        val stmt_tms = fst (listSyntax.dest_list stmts);
        val target =
            if is_BStmt_Jmp est andalso (is_BLE_Label o dest_BStmt_Jmp) est then
                dest_BLE_Label (dest_BStmt_Jmp est)
            else
                raise ERR "is_self_loop_call_site" "not a direct jump";
    in
        identical target lbl_tm andalso List.exists stmt_assigns_link_register stmt_tms
    end
    handle HOL_ERR _ => false;

fun pc_type_with_stubbed_self_loop_calls bl_dict adr_dict lbl_tm syst =
    let
        val pc_type = bir_symbexec_oracleLib.fun_oracle adr_dict lbl_tm syst;
    in
        if pc_type = "Normal" andalso
           bir_symbexec_oracleLib.get_stub_unclassified_calls () then
            case lookup_block_dict bl_dict lbl_tm of
                SOME bl =>
                    if is_self_loop_call_site bl lbl_tm then "Library" else pc_type
              | NONE => pc_type
        else
            pc_type
    end;
...
(* execution of a whole block *)
	...
	let val lbl_tm = SYST_get_pc syst; in
		let
		    val pc_type = pc_type_with_stubbed_self_loop_calls bl_dict adr_dict lbl_tm syst;
