...
local
  ...
  datatype bir_smt_result = BirSmtSat | BirSmtUnsat | BirSmtUnknown;
  datatype bir_smt_type = SMTTY_Bool | SMTTY_BV of int | SMTTY_MEM of (int * int);
  fun smt_type_to_string SMTTY_Bool = "Bool"
    | smt_type_to_string (SMTTY_BV n) = "BV" ^ Int.toString n
    | smt_type_to_string (SMTTY_MEM (a, v)) = "MEM" ^ Int.toString a ^ "_" ^ Int.toString v;
  fun smtlib_vars_compare ((an:string, aty:bir_smt_type), (bn, bty)) =
    if an = bn then String.compare (smt_type_to_string aty, smt_type_to_string bty)
    else String.compare (an, bn);
  val bir_smtLib_z3_prelude = "" : string;
  fun bexp_to_smtlib (conds : (string * bir_smt_type) list)
                      (vars : (string * bir_smt_type) Redblackset.set)
                      (exp : term) =
    (conds, vars, ("true", SMTTY_Bool))
    : ((string * bir_smt_type) list *
       (string * bir_smt_type) Redblackset.set *
       (string * bir_smt_type));
  fun querysmt (_:string) (_:(string * bir_smt_type) Redblackset.set)
               (_:(string * bir_smt_type) list) = BirSmtUnknown;
