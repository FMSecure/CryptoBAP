...
(* Library arity is now read from pipeline.yaml via pipelineConfigLib *)
...
fun compute_inputs_mem n syst =
	...
	val be_mem =
	    (symbval_bexp o get_state_symbv " be not found " bv_mem) syst
	    handle e =>
	      mk_BExp_Store
	        (mk_BExp_Den (``BVar "sy_MEM" (BType_Mem Bit64 Bit8)``),
	         ``BExp_Den (BVar "sy_SP_EL0" (BType_Imm Bit64))``,
	         ``BEnd_LittleEndian``,
	         ``BExp_Den (BVar "sy_MEM_input" (BType_Imm Bit64))``);
...
fun compute_inputs_op_mem n syst =
	...
	val be_mem =
	    ...
	    handle e =>
	      mk_BExp_Store
	        (mk_BExp_Den (``BVar "sy_Op_MEM" (BType_Mem Bit64 Bit8)``),
	         ``BExp_Den (BVar "sy_SP_EL0" (BType_Imm Bit64))``,
	         ``BEnd_LittleEndian``,
	         ``BExp_Den (BVar "sy_Op_MEM_input" (BType_Imm Bit64))``);
...
fun Event lib_type syst =
	...
	val event_names = pipelineConfigLib.get_event_names ();

	val systs = if ((not o List.null) event_names andalso hd(event_names) = "HMAC_receive")
		    then
			(state_branch_simp
			 "event"
			 (``bir_exp_true``)
...
fun Encryption syst =
	...
	val n = pipelineConfigLib.get_library_arity ();
...
fun C_Lib syst =
    let
	val ret = get_bvar_fresh (bir_envSyntax.mk_BVar_string ("C_Lib", “BType_Imm Bit64”));

	val syst = update_envvar ``BVar "R0" (BType_Imm Bit64)`` ret syst;
	val syst = update_symbval (mk_BExp_Den ret) ret syst;
	val syst = add_knowledge_r0 ret syst;
    in
	syst
    end;
