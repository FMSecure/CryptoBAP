...
in (* outermost local *)
  ...
  val allow_unmapped_memory_overapprox = ref false;

  fun set_allow_unmapped_memory_overapprox enabled =
    allow_unmapped_memory_overapprox := enabled;

  fun require_unmapped_memory_overapprox err_src detail =
    if !allow_unmapped_memory_overapprox then ()
    else raise ERR err_src ("unmapped memory over-approximation is disabled: " ^ detail);

  fun overapprox_bexp err_src name sz detail =
    let
      val _ = require_unmapped_memory_overapprox err_src detail;
    in
      get_symbv_bexp_free_sz name sz
    end;
  ...
  fun mem_load_const mem caddr sz_tm =
    ...
    in
      if Arbnum.<= (Arbnum.fromInt (0x40000000), ldaddr) then
        ...
        overapprox_bexp "mem_load_const" "devload" sz "device-memory load"
          ...
          let
            val (exp, deps) = Redblackmap.find (mem_globl, ldaddr)
              handle _ => (
                           ...
                           overapprox_bexp "mem_load_const" "hack_load_global" 32 "global memory is not mapped")
...
*)
    ...
    (var_add_const_match_tm,
      fn res => let
        ...
        val sz = bittype_to_size sz_tm;
      in
        SOME (SymbValBE (overapprox_bexp "mem_load" "memload_pcrel" sz "indirect program-relative load"))
      end),
  ...
  fun mem_store_stack mem bv_sp imm_offset val_tm =
    let
      ...
      val (exp_ml, exp_ml_deps) =
            ...
            handle _ => overapprox_bexp "mem_store_stack" "hack_unmapdstack" 32 "stack memory is not mapped";
  ...
  fun mem_store_const mem caddr val_tm =
    ...
    in
      if Arbnum.<= (Arbnum.fromInt (0x40000000), caddr) then
        (* TODO: peripheral memory range -> add this to memory layout *)
        let
          val _ = require_unmapped_memory_overapprox "mem_store_const" "device-memory store";
        in
          SymbValMem mem
        end
