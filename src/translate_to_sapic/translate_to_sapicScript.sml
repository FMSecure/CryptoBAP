...
val sapic_renaming_dom_update_symb_interpr_update_thm = store_thm(
  "sapic_renaming_dom_update_symb_interpr_update_thm",
  ``
  ∀Re H b t exp.
      sapic_renaming_dom Re =
      IMAGE translate_birvar_to_sapicvar (symb_interpr_dom H)
      ⇒
      sapic_renaming_dom
        (sapic_renaming_update Re (translate_birvar_to_sapicvar b, SOME t)) =
      IMAGE translate_birvar_to_sapicvar
        (symb_interpr_dom (symb_interpr_update H (b, SOME exp)))``,
  Cases >>
  Cases >>
  rw[sapic_renaming_dom_def,sapic_renaming_update_def,
     symb_interpr_dom_def,symb_interpr_update_def,EXTENSION,UPDATE_def,IN_IMAGE] >>
  metis_tac[]);

val sapic_renaming_dom_update_Adv_symb_interpr_update_thm = store_thm(
  "sapic_renaming_dom_update_Adv_symb_interpr_update_thm",
  ``
  ∀Re H t exp.
      sapic_renaming_dom Re =
      IMAGE translate_birvar_to_sapicvar (symb_interpr_dom H)
      ⇒
      sapic_renaming_dom (sapic_renaming_update Re (Var "Adv" 0, SOME t)) =
      IMAGE translate_birvar_to_sapicvar
        (symb_interpr_dom
          (symb_interpr_update H (BVar "Adv" (BType_Imm Bit64), SOME exp)))``,
  rw[] >>
  MP_TAC
    (Q.SPECL [`Re`,`H`,`BVar "Adv" (BType_Imm Bit64)`,`t`,`exp`]
       sapic_renaming_dom_update_symb_interpr_update_thm) >>
  ASM_SIMP_TAC (srw_ss()) [translate_birvar_to_sapicvar_def]);

val sapic_renaming_dom_update_RNG_symb_interpr_update_thm = store_thm(
  "sapic_renaming_dom_update_RNG_symb_interpr_update_thm",
  ``
  ∀Re H t exp.
      sapic_renaming_dom Re =
      IMAGE translate_birvar_to_sapicvar (symb_interpr_dom H)
      ⇒
      sapic_renaming_dom (sapic_renaming_update Re (Var "RNG" 0, SOME t)) =
      IMAGE translate_birvar_to_sapicvar
        (symb_interpr_dom
          (symb_interpr_update H (BVar "RNG" (BType_Imm Bit64), SOME exp)))``,
  rw[] >>
  MP_TAC
    (Q.SPECL [`Re`,`H`,`BVar "RNG" (BType_Imm Bit64)`,`t`,`exp`]
       sapic_renaming_dom_update_symb_interpr_update_thm) >>
  ASM_SIMP_TAC (srw_ss()) [translate_birvar_to_sapicvar_def]);

val sapic_renaming_dom_update_let_symb_interpr_update_thm = store_thm(
  "sapic_renaming_dom_update_let_symb_interpr_update_thm",
  ``
  ∀Re H t exp.
      sapic_renaming_dom Re =
      IMAGE translate_birvar_to_sapicvar (symb_interpr_dom H)
      ⇒
      sapic_renaming_dom (sapic_renaming_update Re (Var "let" 0, SOME t)) =
      IMAGE translate_birvar_to_sapicvar
        (symb_interpr_dom
          (symb_interpr_update H (BVar "let" (BType_Imm Bit64), SOME exp)))``,
  rw[] >>
  MP_TAC
    (Q.SPECL [`Re`,`H`,`BVar "let" (BType_Imm Bit64)`,`t`,`exp`]
       sapic_renaming_dom_update_symb_interpr_update_thm) >>
  ASM_SIMP_TAC (srw_ss()) [translate_birvar_to_sapicvar_def]);
...
(*end of P2A*)
  >-(
	...
	TRY (Q.EXISTS_TAC `symbtree_to_sapic Tree'`) >>
	TRY (Q.EXISTS_TAC `i + 1`) >>
	TRY (Q.EXISTS_TAC
	  `sapic_renaming_update Re
	     (translate_birvar_to_sapicvar (BVar "Adv" (BType_Imm Bit64)),
	      SOME (TVar (translate_birvar_to_sapicvar b)))`) >>
		TRY (Q.EXISTS_TAC `NRe`) >>
		TRY (Q.EXISTS_TAC `Fact InFact [TVar (translate_birvar_to_sapicvar b)]`) >>
		rw[sim_def,sapic_position_transition_def,sapic_position_in_transition_def] >>
		TRY (IMP_RES_TAC position_of_val_thm >>
		     ASM_SIMP_TAC (srw_ss()) [position_in_tree_def]) >>
		TRY (IMP_RES_TAC env_of_val_thm >>
		     FULL_SIMP_TAC (srw_ss()) [env_of_tree_def] >>
		     MATCH_MP_TAC sapic_renaming_dom_update_Adv_symb_interpr_update_thm >>
		     ASM_REWRITE_TAC []) >>
		ASM_SIMP_TAC (srw_ss()) [translate_birvar_to_sapicvar_def])
(*end of A2P*)
  >-(
...
rw[single_step_execute_symbolic_tree_def]>>
	IMP_RES_TAC sim_def>>
		...
		rw[sim_def,sapic_position_transition_def,sapic_position_new_transition_def] >>
		TRY (IMP_RES_TAC position_of_val_thm >>
		     ASM_SIMP_TAC (srw_ss()) [position_in_tree_def]) >>
		TRY (IMP_RES_TAC env_of_val_thm >>
		     FULL_SIMP_TAC (srw_ss()) [env_of_tree_def] >>
		     MATCH_MP_TAC sapic_renaming_dom_update_RNG_symb_interpr_update_thm >>
		     ASM_REWRITE_TAC []))
(*end of Sync_Fr*)
  >-(
    rewrite_tac[sbirEvent_to_sapicFact_def] >>
...
rw[single_step_execute_symbolic_tree_def]>>
	...
	TRY (Q.EXISTS_TAC
	  `sapic_renaming_update Re
	     (Var "let" 0, SOME (translate_birexp_to_sapicterm (BExp_Den b)))`) >>
		TRY (Q.EXISTS_TAC `NRe`) >>
		rw[sim_def,sapic_position_transition_def,sapic_position_let_true_transition_def,
		   sapic_position_let_false_transition_def] >>
		TRY (IMP_RES_TAC position_of_val_thm >>
		     ASM_SIMP_TAC (srw_ss()) [position_in_tree_def]) >>
		TRY (IMP_RES_TAC env_of_val_thm >>
		     FULL_SIMP_TAC (srw_ss()) [env_of_tree_def] >>
		     MATCH_MP_TAC sapic_renaming_dom_update_let_symb_interpr_update_thm >>
		     ASM_REWRITE_TAC []))
  (*end of Crypto*)
  >-(
    rewrite_tac[sbirEvent_to_sapicFact_def] >>
